...
from Sound2Scene.a2s_dataloader import GetVGGSound
...
def save_progress(model, save_path_prefix):
    logger.info("Saving model progress...")
    # Save the embedder
    embedder_path = f"{save_path_prefix}_embedder.bin"
    torch.save(model.embedder.state_dict(), embedder_path)
    logger.info(f"Embedder saved to {embedder_path}")

    # Save the UNet model
    unet_path = f"{save_path_prefix}_unet.bin"
    torch.save(model.unet.state_dict(), unet_path)
    logger.info(f"UNet model saved to {unet_path}")
...
def train():
    ...
    train_dataset = GetVGGSound(args.data_dir)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            #batch:(index, spec, emb_img, orig_img)
            
            with accelerator.accumulate(model):

                # Convert images to latent space
                latents = accelerator.unwrap_model(model).vae.encode((batch[2].squeeze(1)).to(dtype=weight_dtype)).latent_dist.sample().detach()
                ...
                audio_values = Variable(batch[1]).to(accelerator.device).to(dtype=weight_dtype)
                
                _, aud_features = accelerator.unwrap_model(model).aud_encoder(audio_values.unsqueeze(1).float())
                print(aud_features.shape)
                encoder_hidden_states = accelerator.unwrap_model(model).embedder(aud_features.unsqueeze(1)).unsqueeze(1)
                # print(audio_values.shape, aud_features.shape, audio_token.shape)
                ...
                # encoder_hidden_states = aud_features
                ...
                if len(aud_features.shape) > 2:
                    norm_dim = 2
                else:
                    norm_dim = 1

                # add regularization
                reg_loss = args.lambda_a * torch.mean(torch.abs(encoder_hidden_states)) + \
                        args.lambda_b * (torch.norm(encoder_hidden_states, p=2, dim=norm_dim)**2).mean()
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.save_steps == 0:
                    ...
                    if accelerator.is_main_process:
                        save_path_prefix = os.path.join(args.output_dir, f"weights/{args.run_name}-{global_step}")
                        save_progress(model, save_path_prefix)
                        if args.lora:
