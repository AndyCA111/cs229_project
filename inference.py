...
from pipeline import soundiffusionPipeline
...
def inference(args):
    ...
    # )
    test_dataset = GetVGGSound(args.data_dir)
    ...
    for step, batch in enumerate(test_dataloader):
        ...
        audio_values = batch[1].to(accelerator.device).to(dtype=weight_dtype)
        ...
        pipeline = soundiffusionPipeline.from_pretrained(
        ...
        pipeline.unet.load_state_dict(torch.load('/home/binxu/soundiffusion/output/weights/AudioToken-10000_unet.bin'))
        image = pipeline(emb_h_s=encoder_hidden_states,prompt=prompt, num_inference_steps=args.num_inference_steps, guidance_scale=7.5).images[0]
        image.save(f'output/imgs/{args.run_name}/{batch[0]}.png')
